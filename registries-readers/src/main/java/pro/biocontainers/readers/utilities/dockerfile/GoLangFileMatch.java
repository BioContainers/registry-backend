...
public class GoLangFileMatch {
    ...
    public static boolean match(List<String> patterns, File file) throws FileException {
        return !match(patterns, file.getPath()).isEmpty();
    }

    public static boolean match(String pattern, File file) throws FileException {
     ...
     */
    public static List<String> match(List<String> patterns, String name) throws FileException {
    ...
    public static boolean match(String pattern, String name) throws FileException {
        return buildPattern(pattern).matcher(name).matches();
    }

    private static Pattern buildPattern(String pattern) throws FileException {
    ...
    private static String appendChunkPattern(StringBuilder patternStringBuilder, String pattern) throws FileException {
        ...
        for (i = 0; i < pattern.length(); i++) {
            ...
            switch (c) {
                ...
                case '[':
                    if (!isEsc) {
                        if (inRange) {
                            throw new FileException("[ not expected, closing bracket ] not yet reached");
                        }
                        rangeFrom = i;
                ...
                case ']':
                    if (!isEsc) {
                        if (!inRange) {
                            throw new FileException("] is not expected, [ was not met");
                        }
                        if (rangeParseState == RangeParseState.CHAR_EXPECTED_AFTER_DASH) {
                            throw new FileException("Character range not finished");
                        }
                    ...
                    if (!inRange) {
                        ...
                        if (!isEsc) {
                            if (rangeParseState != RangeParseState.CHAR_OR_DASH_EXPECTED) {
                                throw new FileException("- character not expected");
                            }
        ...
        if (isEsc) {
            throw new FileException("Escaped character missing");
        }
        if (inRange) {
            throw new FileException("Character range not finished");
        }
